...
class PersonFragment : Fragment() {
            ...
            .enqueue(object : Callback<ApiResponseUser> {
                override fun onResponse(
                    ...
                    Glide.with(requireContext())
                        .load(user.avatar.replace("localhost", Config.LOCALHOST))
                        .error(R.drawable.avatar_default).into(binding.profileImage)
    ...
    private fun uploadImageToApi(imagePath: String) {
        ...
        ApiServiceUser.apiServiceUser.upLoadAvatar(id, token, imagePart)
            .enqueue(object : Callback<Void> {
                override fun onResponse(call: Call<Void>, response: Response<Void>) {
                    Toast.makeText(requireActivity(), "update avatar success", Toast.LENGTH_SHORT)
                        .show()
                    getProfile(Constant.token)
                }

                override fun onFailure(call: Call<Void>, t: Throwable) {
                    Toast.makeText(requireActivity(), "update avatar fail", Toast.LENGTH_SHORT)
                        .show()
